...
# Executes docker-compose to migrate an existing database.
...
docker-compose -f ${APREXIS_DOCKER_COMPOSE_FILE} \
    run -T --no-deps --rm ${APREXIS_SHELL} bash --login -c "/aprexis/setup-for-rails.sh; bundle exec rails db:migrate"
